...
int			row_malloc(int max_row, t_param *param)
{
	if (!(g_map = (int **)malloc(sizeof(int *) * (max_row + 1))))
		return (error(param, 5));
	g_map[max_row] = 0;
	param->mapheight = max_row;
...
int			put_map(int row, int max_row, t_param *param, char *line)
{
	int		i;
	int		col;
	int		num;

	if (!row && !row_malloc(max_row, param))
		return (0);
...
int			read_map(char *file, t_param *param)
{
	...
	if (!(max_row = row_count(file, param)))
		return (error(param, 4));
	if ((fd = open(file, O_RDONLY)) < 0)
		return (error(param, 4));
	row = 0;
	...
	{
		if (line[0] == 'R')
			parse_r(&max_row, param, line);
		else if (line[0] == 'F')
			parse_f(&max_row, param, line);
		else if (line[0] == 'C')
			parse_c(&max_row, param, line);
		else
			parse(&row, &max_row, param, line);
	}
	return (row);
}
