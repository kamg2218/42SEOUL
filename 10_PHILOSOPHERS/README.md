...
###### pthread_create
    ...
    스레드를 만드는 데에 성공하면 0을, 실패하면 -1을 반환한다.
...
###### pthread_detach
    ...
    감지된 스레드를 종료할 때, 스레드의 자원도 다른 스레드를 기다리지 않고, 자동으로 정리된다.
    
    이미 감지된 스레드를 감지하려고 하는 것은 정의되지 않은 행동으로, 무한 루프에 빠질 수 있다.
    
   ** 스레드가 종료되는 경우, 다른 스레드를 기다리지 않고 종료된다는 것에 주의해야한다!(모든 스레드가 정리될 수 있음!)
