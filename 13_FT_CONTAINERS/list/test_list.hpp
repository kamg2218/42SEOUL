...
//constructor
template <class T, class Allocator>
ft::list<T, Allocator>::list() : sz(0), head(0) {
	add_back(0);
	//add_back(0);
}
...
template <class T, class Allocator>
ft::list<T, Allocator>::list(const Allocator& alloc) : sz(0), head(0) { add_back(0); }
...
template <class T, class Allocator>
typename ft::list<T, Allocator>::iterator	ft::list<T, Allocator>::end() { return iterator(this->head); }
template <class T, class Allocator>
typename ft::list<T, Allocator>::const_iterator		ft::list<T, Allocator>::end() const { return const_iterator(this->head); }
template <class T, class Allocator>
typename ft::list<T, Allocator>::reverse_iterator	ft::list<T, Allocator>::rbegin() { return reverse_iterator(this->head->prev); }
template <class T, class Allocator>
typename ft::list<T, Allocator>::const_reverse_iterator	ft::list<T, Allocator>::rbegin() const { return const_reverse_iterator(this->head->prev); }
template <class T, class Allocator>
...
template <class T, class Allocator>
typename ft::list<T, Allocator>::reference	ft::list<T, Allocator>::back() { return this->head->prev->value; } //this->tail->prev->value; }
template <class T, class Allocator>
typename ft::list<T, Allocator>::const_reference	ft::list<T, Allocator>::back() const { return this->head->prev->value; }
...
void		ft::list<T, Allocator>::sort(){
	...
	for (ft::node<T>* i = this->head->next; i != this->head; i = i->next){
		for (ft::node<T>* j = i->next; j != this->head; j = j->next){
			if (i->value > j->value){
				tmp = j->next;
				move_node(j, i);
				move_node(i, tmp);
...
void	ft::list<T, Allocator>::merge(ft::list<T, Allocator>& other){
	...
	i = other.head->next;
	while (i != other.head){
		if (tmp->value > i->value || tmp == this->head){
			move_node(i, tmp);
			other.sz--;
			this->sz++;
			i = other.head->next;
...
void	ft::list<T, Allocator>::merge(ft::list<T, Allocator>& other, Compare comp){
	...
	i = other.head->next;
	while (i != other.head){
		if (comp(tmp->value, i->value) > 0 || tmp == this->head){
			move_node(i, tmp);
			other.sz--;
			this->sz++;
			i = other.head->next;
