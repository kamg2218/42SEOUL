...
bool	operator==(const ft::stack<T,Container>& lhs, const ft::stack<T,Container>& rhs){
	typename Container::reverse_iterator	i;
	typename Container::reverse_iterator	tmp;

	if (lhs.size() != rhs.size())
		return false;
	i = lhs.getContainer().rbegin();
	tmp = rhs.getContainer().rbegin();
	for (; i != lhs.getContainer().rend() && tmp != rhs.getContainer().rend(); i++, tmp++){
		if (*i != *tmp)
			return false;
	}
	return true;
